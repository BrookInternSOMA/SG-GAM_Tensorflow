...
def generator_unet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        d1 = deconv2d(tf.nn.relu(e5), options.gf_dim * 8, name='g_d1')
        d1 = tf.nn.dropout(d1, dropout_rate)
        d1 = tf.concat([instance_norm(d1, 'g_bn_d1'), e4], 3)

        d2 = deconv2d(tf.nn.relu(d1), options.gf_dim*4, name='g_d2')
        d2 = tf.nn.dropout(d2, dropout_rate)
        d2 = tf.concat([instance_norm(d2, 'g_bn_d2'), e3], 3)

        d3 = deconv2d(tf.nn.relu(d2), options.gf_dim*2, name='g_d3')
        d3 = tf.nn.dropout(d3, dropout_rate)
        d3 = tf.concat([instance_norm(d3, 'g_bn_d3'), e2], 3)

        d4 = deconv2d(tf.nn.relu(d3), options.gf_dim, name='g_d4')
        d4 = tf.concat([instance_norm(d4, 'g_bn_d4'), e1], 3)

        d5 = deconv2d(tf.nn.relu(d4), options.output_c_dim, name='g_d5')
        """
